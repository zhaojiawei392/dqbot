...
timr::SerialManipulator<Scalar, size> construct_robot_from_args() {
    ...
    timr::SerialManipulator<float, 6> robot(dh, limits, joint_positions);
    return robot;
}


template<typename Scalar, std::size_t size>
timr::SerialManipulator<Scalar, size> construct_robot_from_json() {
    const std::string json_path("../examples/robot.json");
    const std::array<float, 6> joint_positions {0,0,0,0,0,0};

    timr::SerialManipulator<float, 6> robot(json_path, joint_positions);
    return robot;
}
